...
If the solution to the problem is \(C\), and one of its activities ends at \(f_x\), then the subset of \(C\) whose activities all finish before or at \(f_x\) must be an optimal solution to a smaller problem where its resource has a finish time of \(f_x\). Similarly, the subset of \(C\) whose activities all start after or at \(f_x\) must be a solution to the smaller problem where the resource has a start time of \(f_x\).

\subsection{A Recursive Formula}

Let \(C_{S,F}\) denote the set of compatible activities in \(A_{S,F}\) such that \(T_{S,F}\) is maximal (where \(T_{S,F}\) is the amount of time that the activities in \(C_{S,F}\) are running).
\begin{equation*}
	C_{S,F} =
	\begin{cases}
		\\
		\displaystyle\max_{a_i \in A_{S,F}} ()
	\end{cases}
\end{equation*}
