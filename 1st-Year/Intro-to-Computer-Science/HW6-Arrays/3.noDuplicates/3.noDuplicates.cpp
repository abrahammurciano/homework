 ...
 * Program Description:		Reads in values until a 0 is read in, then
 * removes all duplicate entries Course Name:
 * Introduction to Computer Science Assignment Number:		6 Question
 * Number:			3
...
// Function that deletes duplicate values of array of length n, and adjusts n to
// new size
void delDup(int array[], int &n) {
	...
	for (int i = 1; i < n; i++) {
		...
		for (int j = 0; j < i; j++) {
			...
			// Check if the ith one is a repetition of a previous one. If so, delete
			// it
			if (array[i] == array[j]) {
...
// Function that prints the values of a 1D array of integers separated by a
// space
